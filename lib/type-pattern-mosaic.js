...
import { renderTypePattern, PATTERN_TYPES, TYPE_PATTERN_DEFAULTS } from './type-pattern.js';
...
export function randomizePatternOptions(letter, constraints = {}, rng = Math.random) {
  ...
  return {
    ...
    fillAngle: int(-28, 28),
    padding: int(6, 20),
    backgroundColor: rest.backgroundColor ?? TYPE_PATTERN_DEFAULTS.backgroundColor,
    foregroundColor: rest.foregroundColor ?? TYPE_PATTERN_DEFAULTS.foregroundColor,
    ...rest
  };
}
...
function withSharedColors(cellOpts, sharedOptions = {}) {
  const out = { ...cellOpts };
  if (sharedOptions.backgroundColor != null) out.backgroundColor = sharedOptions.backgroundColor;
  if (sharedOptions.foregroundColor != null) out.foregroundColor = sharedOptions.foregroundColor;
  return out;
}
...
function resolveTilePattern(letter, opts, rng) {
  ...
  if (opts.randomize) {
    return withSharedColors(randomizePatternOptions(char, opts.sharedOptions, rng), opts.sharedOptions);
  }
...
function resolveMosaicGrid(w, h, opts) {
  ...
  const cellW = gridCols > 1 ? (w - gap * (gridCols - 1)) / gridCols : w;
  const cellH = gridRows > 1 ? (h - gap * (gridRows - 1)) / gridRows : h;
...
export function renderTypePatternMosaic(canvas, letter, options = {}) {
  ...
  const char = String(letter || 'A').slice(0, 1) || 'A';
  const sharedTileOpts = opts.sameTilePattern ? resolveTilePattern(char, opts, rng) : null;
  ...
  for (let row = 0; row < gridRows; row++) {
    for (let col = 0; col < gridCols; col++) {
      ...
      if (sharedTileOpts) {
        ...
        cellOpts = withSharedColors({ letter: char, ...opts.sharedOptions }, opts.sharedOptions);
      }

      renderTypePattern(tileCanvas, { ...cellOpts, width: cellW, height: cellH });
