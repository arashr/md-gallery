...
export function buildCodeStylesheetForExport(cfg = activeConfig) {
  ...
  for (const [name, def] of Object.entries(getGroundDefs(cfg))) {
    ...
    const inlineBg = mixHexRgb(codeBg, surface, inlineTowardSurface);
    lines.push(
      `.ground-${name}{--on-ground-code-bg:${codeBg};}`,
      `.ground-${name} .prose pre{--code-block-bg:var(--on-ground-code-bg);}`,
      `.post-card.ground-${name} .prose :not(pre)>code{background:${inlineBg};}`
    );
  }
...
export function buildExportRootVars(cfg = activeConfig) {
  ...
  return (
    ...
    `--config-grain-tile-size:${grain.tileSize ?? '96px'};` +
