...
export function buildGroundStylesheet(cfg) {
  ...
  const glyphOpacity = Math.min(1, Math.max(0, Number(graphics.glyphPatternOpacity) || 0.07));
  const lines = [];

  for (const [name, def] of Object.entries(defs)) {
    const fg = def.foreground;
    const glyphColor =
      String(graphics.glyphPatternColor ?? 'display') === 'display'
        ? resolveColor(fg.display, cfg)
        : resolveColor(graphics.glyphPatternColor, cfg);
    lines.push(
...
export function buildCodeStylesheetForExport(cfg = activeConfig) {
  ...
  for (const [name, def] of Object.entries(getGroundDefs(cfg))) {
    ...
    lines.push(
      ...
      `.ground-${name} .prose pre{--code-block-bg:var(--on-ground-code-bg);background:transparent;}`,
