...
const DEFAULT_CONFIG = {
  ...
  fonts: {
    ...
    titleFaces: [
      ...
      { id: 'limelight', google: 'BBH+Bogle', lineHeight: '0.8', letterSpacing: '0.01em' },
      { id: 'jersey25', google: 'Jersey+25', lineHeight: '0.7', letterSpacing: '-0.01em' },
      { id: 'blackops', google: 'Black+Ops+One', lineHeight: '0.86', letterSpacing: '0.02em' },
      { id: 'calsans', google: 'Cal+Sans', lineHeight: '0.94', letterSpacing: '-0.01em' }
    ]
  },
  titleScale: {
    minPx: 64,
...
export async function loadGalleryConfig(url = 'config/gallery.config.json', { bust = false } = {}) {
  try {
    const fetchUrl = bust ? `${url}${url.includes('?') ? '&' : '?'}v=${Date.now()}` : url;
    const res = await fetch(fetchUrl, bust ? { cache: 'no-store' } : undefined);
...
export async function reloadGalleryConfig(url = 'config/gallery.config.json', opts = {}) {
  const cfg = await loadGalleryConfig(url, { bust: true });
  applyGalleryConfigToDocument(cfg, opts);
  return cfg;
}

/** @param {GalleryConfig} cfg @param {{ scope?: 'landing' | 'full' }} [opts] */
export function fontsHrefFromConfig(cfg = activeConfig, opts = {}) {
  const scope = opts.scope === 'full' ? 'full' : 'landing';
  const { fonts } = cfg;
  const families = [
    fonts.uiSans.google,
    fonts.mono.google,
  ...
  if (scope === 'full') {
    families.push(fonts.uiSerif.google);
  }
  const unique = [...new Set(families.filter(Boolean))];
  return `https://fonts.googleapis.com/css2?family=${unique.join('&family=')}&display=swap`;
}

/** Upgrade the Google Fonts link when the reader opens (adds ui serif). */
export function loadReaderFonts(cfg = activeConfig) {
  const link = document.getElementById('fonts-link');
  if (!link) return;
  const href = fontsHrefFromConfig(cfg, { scope: 'full' });
...
export function applyGalleryConfigToDocument(cfg = activeConfig, opts = {}) {
  const fontScope = opts.fontScope === 'full' ? 'full' : 'landing';
  ...
  const glyphColor =
    glyphTokens.color === 'display'
      ? readRootToken('--hero-display', themeColors().red)
      : resolveColor(glyphTokens.color, cfg);
  root.style.setProperty('--glyph-pattern-color', glyphColor);
  root.style.setProperty('--glyph-pattern-opacity', String(glyphTokens.opacity));

  const iso = resolveImageIsometricOptions(cfg);
  const isoVarNames = Object.keys(imageIsometricCssVars(IMAGE_ISO_DEFAULTS));
  if (iso.tableChrome) {
    root.dataset.imageFrame = 'table';
    for (const name of isoVarNames) {
      root.style.removeProperty(name);
    }
  } else {
    root.dataset.imageFrame = 'off';
    for (const name of isoVarNames) {
      root.style.removeProperty(name);
    }
  }
  ...
  const link = document.getElementById('fonts-link');
  if (link) link.href = fontsHrefFromConfig(cfg, { scope: fontScope });
