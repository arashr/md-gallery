...
export function parseDocument(rawText, filename = 'document.md') {
  ...
  let posters = rawBlocks.map((block, index) => {
    ...
    return {
      ...
      slug,
      bodyMarkdown,
      searchText: `${plainTitle} ${bodyMarkdown}`.toLowerCase()
    };
  });

  ({ introMarkdown, posters } = maybePromoteIntroToPoster(introMarkdown, posters, usedSlugs));
...
/** Long or structured pre-## content becomes the first poster instead of hero intro. */
function maybePromoteIntroToPoster(introMarkdown, posters, usedSlugs) {
  const intro = introMarkdown?.trim();
  if (!intro) return { introMarkdown: intro, posters };

  const introLines = intro.split(/\r?\n/);
  const introFlags = lineFlags(introLines);
  const lineCount = introLines.filter((l, i) => l.trim() && !introFlags[i]).length;
  let substantial = lineCount > 4 || intro.length > 400;
  if (!substantial) {
    for (let i = 0; i < introLines.length; i++) {
      if (!introFlags[i] && /^#{2,3}\s/.test(introLines[i])) {
        substantial = true;
        break;
      }
    }
  }
  if (!substantial) return { introMarkdown: intro, posters };

  let overviewTitle = 'Overview';
  let h2;
  let h3;
  for (let i = 0; i < introLines.length; i++) {
    if (introFlags[i]) continue;
    if (!h2) {
      const m = introLines[i].match(/^##\s+(.+)$/);
      if (m) h2 = m;
    }
    if (!h3) {
      const m = introLines[i].match(/^###\s+(.+)$/);
  ...
  if (h2) overviewTitle = h2[1].trim();
  else if (h3) overviewTitle = h3[1].trim();

  const bodyMarkdown = stripLeadingHeadingFromIntro(intro, overviewTitle);
  const overview = {
    index: 0,
    title: overviewTitle,
    plainTitle: plainTextFromMarkdown(overviewTitle),
    slug: uniqueSlug(slugify(plainTextFromMarkdown(overviewTitle)), usedSlugs),
    bodyMarkdown,
    searchText: `${plainTextFromMarkdown(overviewTitle)} ${bodyMarkdown}`.toLowerCase()
  };

  const shifted = posters.map((p, i) => ({ ...p, index: i + 1 }));
  return { introMarkdown: '', posters: [overview, ...shifted] };
}
