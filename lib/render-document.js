...
function mdToHtml(markdown) {
  ...
  return sanitizeHtml(marked.parse(markdown.trim()));
}
...
export function renderDocument(doc, fileLabel) {
  ...
  const postersHtml = doc.posters
    .map((poster) => {
      ...
      return `
    <article class="post-card ${ground} reveal title-face-${face.id} is-visible" id="${escapeHtml(poster.slug)}" data-slug="${escapeHtml(poster.slug)}" data-search="${escapeHtml(poster.searchText)}" style="--poster-shift: ${staggerRem}rem">
      <header class="post-header">
        <h2 class="poster__title post-title"><a href="#${escapeHtml(poster.slug)}">${escapeHtml(poster.title)}</a></h2>
      </header>
  ...
  return `
    <header class="poster ground-lilac collection-hero reveal is-visible">
      <p class="kicker">MD Gallery</p>
      <h1 class="poster__title">${escapeHtml(doc.title)}</h1>
...
export function renderToc(toc) {
  ...
  return `<ol class="toc-list">
    ${toc
      .map(
        (item) =>
          `<li class="toc-item toc-depth-${item.depth}"><a href="#${escapeHtml(item.id)}" data-toc-link>${escapeHtml(item.text)}</a></li>`
      )
      .join('\n')}
  </ol>`;
}
