...
export function renderMiniPoster(opts) {
  ...
  const ground = groundIn || groundForSlug(slug);
  const face = titleFaceId ? { id: titleFaceId } : titleFaceForIndex(index);
...
export function renderMiniPosterGrid(items) {
  return items
    .map(({ path, title, subtext, index }) =>
      renderMiniPoster({
        slug: slugify(path.replace(/\.[^.]+$/, '')) || 'file',
        title,
