...
/** Accent on these grounds is ornamental (matches md-portfolio CSS tokens), not a body-text pair. */
const DECORATIVE_ACCENT_GROUNDS = new Set(['lime', 'butter']);
...
export function auditGroundForegrounds(cfg) {
  ...
  for (const [name, def] of Object.entries(defs)) {
    for (const role of ['body', 'muted', 'display', 'accent', 'focus']) {
      if (role === 'accent' && DECORATIVE_ACCENT_GROUNDS.has(name)) continue;
