...
import { getGroundDefs, resolveColor, codeBlockBgFromSurface } from './gallery-config.js';
...
export function auditGroundForegrounds(cfg) {
  ...
  for (const [name, def] of Object.entries(defs)) {
    ...
    const codeText = cfg.theme?.code?.text ?? 'paper';
    const codeBg = codeBlockBgFromSurface(def.surface, cfg);
