...
export function computePosterGlyphRegionFromLayout(layout, cfg, rand) {
  ...
  const bandW = Math.min(
    Math.max(24, clampInt(opts.fallbackBandWidth, 88)),
  ...
  if (opts.edgeOverflowPx != null && Number.isFinite(Number(opts.edgeOverflowPx))) {
    ...
    edgeOverflow = Math.max(0, useLeft ? contentLeft : cardWidth - contentRight);
  }

  let x = useLeft ? bandLeft : bandRight - bandW;
  let width = bandW;
  if (!alignEdge && edgeOverflow > 0) {
    if (useLeft) x -= edgeOverflow;
    else width += edgeOverflow;
  }
  if (alignEdge) {
    x = useLeft ? 0 : cardWidth - bandW;
    width = bandW;
  }
