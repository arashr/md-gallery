...
const DEFAULT_PLACEMENT = {
  ...
  edgeOverflowPx: 24,
  /** Bottom slot only: divide min height by this ratio (e.g. 2.5 = easier bottom placement). */
...
export function computePosterGlyphRegionFromLayout(layout, cfg, rand) {
  ...
  const bottomBandHeightRatio = Math.max(1, clampNum(opts.bottomBandHeightRatio, 1));
  ...
  for (const key of pref) {
    ...
    if (region) {
      const minH = key === 'bottom' ? minGapUse / bottomBandHeightRatio : minGapUse;
      if (region.height >= minH && region.width >= 24) {
