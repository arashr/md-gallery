/**
 * `theme.graphics.imageIsometric` — prose image frame (gallery: config-only, on by default).
 */
...
export function resolveImageIsometricOptions(cfg) {
  ...
  const tableChromeRaw = nested.tableChrome ?? graphics.imageIsometricTableChrome;
  const tableChrome =
    tableChromeRaw === true || tableChromeRaw === false
      ? tableChromeRaw
      : IMAGE_ISO_DEFAULTS.tableChrome;
