...
export const PATTERN_TYPES = ['line', 'circle', 'arc', 'spiral', 'wave', 'grid', 'fill'];
...
export function renderTypePattern(canvas, options = {}) {
  ...
  const w = options.width ?? canvas.clientWidth ?? canvas.parentElement?.clientWidth ?? 400;
  const h = options.height ?? canvas.clientHeight ?? canvas.parentElement?.clientHeight ?? 400;
  ...
  const pad = opts.opticalTight ? 0 : opts.padding;
  ...
  const fontSize =
    opts.fontSize ?? estimateFontSize(opts.type, innerW, innerH, repeats, letter, ctx, opts);
  ...
  const placements = computePlacements(opts.type, {
    repeats,
    letter,
    letterWidth,
    fontSize,
    ...
    cx,
    cy,
    opts: { ...opts, padding: pad }
  });

  placements.forEach((p, i) => {
    drawLetter(ctx, letter, p.x, p.y, p.angle, opts, i);
  });
...
function estimateFontSize(type, innerW, innerH, repeats, letter, ctx, opts) {
  ...
  if (type === 'fill' || type === 'grid') {
    ...
    return Math.floor(Math.min(innerW / (cols * 1.1), innerH / (rows * opts.fillRowGap * 1.2)));
  }
...
function linePlacements({ repeats, letterWidth, innerW, innerH, cx, cy, opts }) {
  ...
  const spacing = opts.spacing ?? (opts.opticalTight ? tightStep(letterWidth, opts) : len / Math.max(repeats - 1, 1));
...
function circlePlacements(env, sweep, startAngle = -Math.PI / 2) {
  ...
  const rFromLetters = sweep > 0 ? (repeats * step) / sweep : maxR;
  const r = opts.opticalTight ? Math.min(maxR, rFromLetters) : maxR;
  const out = [];
  for (let i = 0; i < repeats; i++) {
    const t = repeats === 1 ? 0.5 : i / (repeats - 1);
...
function wavePlacements(env) {
  const { repeats, pad, innerW, innerH, cy, letterWidth, opts } = env;
  const amp = innerH * opts.waveAmplitude;
  const cycles = opts.waveCycles;
  const span = opts.opticalTight ? tightStep(letterWidth, opts) * Math.max(repeats - 1, 1) : innerW;
  ...
  for (let i = 0; i < repeats; i++) {
    ...
    const dx = (opts.opticalTight ? span : innerW) / Math.max(repeats - 1, 1);
    const dy = (Math.cos(phase) * amp * (cycles * Math.PI * 2)) / Math.max(repeats - 1, 1);
...
function fillPlacements(env) {
  ...
  for (let row = 0; row < rows && n < repeats; row++) {
    ...
    for (let col = 0; col < cols && n < repeats; col++) {
      ...
      if (x >= pad && x <= pad + innerW && y >= pad && y <= pad + innerH) {
        out.push({
          x,
          y,
          angle: opts.followPath ? angle : 0
...
function drawLetter(ctx, letter, x, y, angle, opts, index = 0) {
  ...
  const flipVertical = alternate && (opts.flipAlternateVertical || opts.flipAlternate === true);
