...
9. **Prose measure cap** — body text targets ~50–75 characters per line (`65ch` default). Poster width (`42rem`) stays decoupled from measure so UI font changes do not shrink cards.
...
11. **Wide screens stagger, narrow screens stack** — gallery offset (`--poster-shift`) only above ~900px; mobile is full‑width, left‑aligned, no horizontal scroll.
...
13. **One display face per poster** — title faces rotate from config (`titleFaces`); in‑body `h2`–`h4` inherit the same face as the poster title.
...
25. **One poster, one editorial unit** — each `##` (or split rule) is a self‑contained card: title block + prose body, same visual language as figlets‑blog.
26. **Sanitize all user HTML** — markdown output always passes through the allowlist sanitizer; design assumes untrusted content.
27. **Sync poster CSS with figlets‑blog manually** — `assets/site.css` is shared DNA; mirror intentional changes there, not via merged builds.

### Dev workflow

28. **Edit JSON, refocus tab** — config hot‑reloads on window focus after save; no rebuild step.
...
Page chrome edges (`--edge-1`, `--hair`) use the same OKLCH mix chain on `--surface` / `--paper`. Tune strength with `theme.layout.edgeStepMix` (0.04–0.4).
...
| Stagger | — | `--poster-shift` from `lib/stagger.js` |
