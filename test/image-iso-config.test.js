...
test('resolveImageIsometricOptions uses defaults', () => {
  ...
  assert.equal(iso.tableChrome, true);
  assert.equal(iso.rotateY, IMAGE_ISO_DEFAULTS.rotateY);
  assert.equal(iso.stroke, '#000000');
});

test('resolveImageIsometricOptions enables table chrome from config', () => {
  const iso = resolveImageIsometricOptions({
    theme: { graphics: { imageIsometric: { tableChrome: true } } }
  });
  assert.equal(iso.tableChrome, true);
});

test('resolveImageIsometricOptions merges isometric overrides', () => {
  const iso = resolveImageIsometricOptions({
    theme: {
      graphics: {
        imageIsometric: {
